...
cities <- tigris::county_subdivisions(
  ...
  #keep northfield, but drop the rest of rice county
  mutate(keep = case_when(NAME == "Northfield" & COUNTYFP == "131" ~ "keep",
                          COUNTYFP == "131" ~ "remove",
                          TRUE ~ "keep")) %>%
  filter(keep == "keep") %>%
    mutate(NAME = case_when(
...
ctu_crosswalk <- find_crosswalks(cities) %>%
...
```{r}
  save(ctu_crosswalk, nhood_crosswalk, wide_ctu_crosswalk, 
       cities, county_outline, 
       file = paste0(here::here(), "/data-raw/geography_data.rda"))
