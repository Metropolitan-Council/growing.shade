...
fheight = if (nrow(params$vars_selected) < 3) {
  3
} else {nrow(params$vars_selected)*.8}
...
This section summarizes the how the tree canopy priority of the selected region compares to other geographies across the region.
 ...
 ```r reportfor``` has an existing tree coverage of approximately ```r round(canopycover$raw_value, 1)*100```%. In most areas in our region, a tree canopy coverage of 40% (as detected by our methods) leads to the greatest benefits. Some areas in our region are dominated by native tallgrass prairie which has lower tree coverage - this should not be penalized. 
...
```{r status, echo=F, message=F, warning=F, fig.height=fheight, fig.width=10}
...
```{r equityfig, echo=F, message = F, warning = F, fig.width=8, fig.height = 4}
...
race_equity <- equityplot%>%
  ggplot(aes(x = pbipoc, y = canopy_percent/2)) + 
  geom_point(col = "grey40", pch = 21, data = filter(equityplot, tract_string != params$selected_tract)) + 
...
inc_equity <- equityplot%>%
  ggplot(aes(x = mdhhincnow, y = (canopy_percent/2))) + 
  geom_point(col = "grey40", pch = 21) + #, data = filter(equityplot, tract_string != params$selected_tract)) + 
...
## Considerations to enhance and maintain tree canopy
...
Invasion by the Emerald ash borer (EAB) insect is a major threat to existing tree canopy. Data shows that EAB has infected ```r eabcount``` trees in the selected geography (<a href = 'https://mnag.maps.arcgis.com/apps/webappviewer/index.html?id=63ebb977e2924d27b9ef0787ecedf6e9' target = '_blank'>Minnesota DNR</a>). Please note that these data are not necessarily intended to identify every infected tree, however we still believe this information to be useful. 
...
```{r biodiv, message = F, warning = F, echo = F, fig.width=8, fig.height = 4}
treebiodiv %>%
...
- if we add something on "tree stress/status" for maintence (variability in ndvi over time?)
