...
filter(testdf, tract_string == testtr) %>%
              select(variable, raw_value, overall_rank) %>%
  rename(`Selected area` = raw_value) %>%
  full_join(metadata %>%
              select(MEANRAW, variable, name)) %>% 
  ungroup() %>%
  select(-variable) %>%
  rename(`Regional average` = MEANRAW,
         Variable = name,
         `Ranking` = overall_rank) %>% 
  mutate(`Ranking` = as.factor(Ranking)) %>%
   mutate(across(where(is.numeric), ~ifelse(str_detect(Variable, "%"), . * 100, .))) %>%
  mutate(across(where(is.numeric), round, 2)) %>%
  select(Variable, `Selected area`, `Regional average`, Ranking)
  # mutate(across(starts_with("%", Variable), ~`Selected area` * 100))

testdf %>%
    filter(tract_string == testtr) %>%
  ggplot(aes(x = weights_nominal))
