...
df1 <- bg_growingshade_main_allMN %>% #bg_growingshade_data_allMN
...
df <- df1 %>%
        dplyr::select(canopy_percent, mdhhincnow, pbipoc) %>%
...
```{r}
hinc <- mn_bgs %>% dplyr::select(GEOID) %>%
...
rl <- bg_growingshade_data %>%
  dplyr::select(tract_string, holc_pred, holc_pgrn, holc_pblu, holc_pylw, canopy_percent, ndvi, avg_temp) %>% #
  mutate_all(~replace(., . == 0, NA)) %>%
  mutate(flag = if_else(is.na(holc_pred) & is.na(holc_pblu) & is.na(holc_pgrn) & is.na(holc_pylw), "remove", "keep")) %>%
  filter(flag == "keep") %>% dplyr::select(-flag) %>%
...
ctu_list %>%
  st_drop_geometry() %>%
  dplyr::dplyr::select(GEO_NAME, canopy_percent, min, max) %>%
  mutate(min = min/100,
         max = max / 100) %>%
