...
mod_report_server <- function(id,
  ...
  # the min, max, ntracts, eab, treeacres, landacres, canopypercent, avgcanopy for the selected geography
      ...
      tagList(HTML(
        paste0(
          if(geo_selections$selected_geo == "tracts") {
            paste0(param_fancytract(), " has an existing tree canopy coverage of ", round(param_areasummary()$canopy_percent * 100, 2), 
    ...
    tree_report_plot <- reactive({
      ...
      if(geo_selections$selected_geo != "tracts") {
        canopyplot<- 
          (as_tibble(if(geo_selections$selected_geo == "ctus") {ctu_list} else {nhood_list}) %>%
