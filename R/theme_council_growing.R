#' @title Council ggplot2 theme, modified for growing shade project
#'
#' @description The default `theme_council_growing()` plus a more simple `theme_council_growing_open()` for making MetCouncil figures. `theme_council_growing()` will be appropriate in most cases while `theme_council_growing_open()` is appropriate for single scatter plots or line graphs. For geospatial plots, `theme_council_growing_geo()` may be useful to set some initial parameters.
#'
...
#'  `theme_council_growing` in the console. You can then copy/paste the
...
#'
#'
theme_council_growing <- function(
    base_size = 11, 
    base_family = "",
    ...
    font_sizes = list(title = 22, subtitle = 16, axis_title = 14, 
                      axis_text = 11, legend_title = 14, legend_text = 10, 
                      caption = 8, strip = 14)) 
{
  purrr::map(c(use_showtext, use_manual_font_sizes), rlang:::check_bool)
  purrr::map(c(base_size), rlang:::check_number_decimal)
  ...
  if (use_showtext == TRUE) {
    ...
    if (grepl("mac", osVersion)) {
      ...
      sysfonts::font_paths(paste0("C:\\Users\\", Sys.info()["user"], 
                                  "\\AppData\\Local\\Microsoft\\Windows\\Fonts"))
    }
    font_locs <- subset(sysfonts::font_files(), family %in% 
                          c("HelveticaNeueLT Std Cn", "HelveticaNeueLT Std Lt", 
                            "Arial Narrow") & face == "Regular")
    purrr::map2(font_locs$family, font_locs$file, sysfonts::font_add)
    font_families <- list(title = "HelveticaNeueLT Std Lt", 
                          subtitle = "HelveticaNeueLT Std Cn", axis_title = "Arial Narrow", 
                          axis_text = "Arial Narrow", legend_title = "HelveticaNeueLT Std Cn", 
                          legend_text = "Arial Narrow", caption = "Arial Narrow", 
                          strip = "Arial Narrow")
  }
  else {
    font_families <- list(title = "sans", subtitle = "sans", 
                          axis_title = "sans", axis_text = "sans", legend_title = "sans", 
                          legend_text = "sans", caption = "sans", strip = "sans")
  }
  if (use_manual_font_sizes == FALSE) {
    font_sizes <- list(title = ggplot2::rel(1.2), subtitle = ggplot2::rel(1.1), 
                       axis_title = base_size, axis_text = ggplot2::rel(0.8), 
  ...
  t <- ggplot2::theme(
    line = ggplot2::element_line(colour = colors$suppBlack, 
                                 ...
                                 linetype = 1, 
                                 lineend = "butt"), 
    rect = ggplot2::element_rect(fill = colors$suppWhite, 
                                 colour = colors$suppBlack,
                                 size = base_rect_size, 
                                 linetype = 1), 
    text = ggplot2::element_text(
      family = base_family,
      face = "plain", colour = colors$suppBlack, size = base_size,
    ...
    axis.text = ggplot2::element_text(
      ...
      size = font_sizes$axis_text, color = "gray30"),
    ...
    axis.title.x = ggplot2::element_text(
      ...
      vjust = -1, family = font_families$axis_title,
    ...
    axis.title.y = ggplot2::element_text(
      angle = 0, family = font_families$axis_title, 
      size = font_sizes$axis_title, 
    ...
    legend.title = ggplot2::element_text(hjust = 0, family = font_families$title, 
