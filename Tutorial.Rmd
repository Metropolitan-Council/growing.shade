...
knitr::opts_chunk$set(echo = F, message = F, warning = F,
                      cache = TRUE,
                      cache.path = "cache/")
library(dplyr); library(tidyr); #library(ggplot2); library(string)
library(tigris)
library(sf)
library(tidycensus)
library(ggbeeswarm)
...
These global parameters will be used in multiple code pieces. 

This code is set up to use/process 2020-era census geographies at the block group level.

```{r}
state <- c("MN")

# either type in specfic county names, or for all counties within a state assign "NULL"
...
acs_year <- 2020 
#note, decennial census data provides race-level information, but ACS provides other data
# check what is the most recent year of acs data here: https://api.census.gov/data.html Search for "acs5" and see what is the most recent year of data available. As of 28 Sept 2022, 2020 was the most recent year available. 
```
...
The demographic information is fetched using the `tidycensus` package. It might be nice to eventually figure out how to natively grab the data via the Census API (rather than going through `tidycensus`), but for now this works well.
