...
This code is set up to use/process 2020-era census geographies at the block group level (even though some datasources are still using 2010-era geographies...but we'll deal with those later).

```{r}
state <- c("MN")

# either type in specific county names, or for all counties within a state assign "NULL"
...
process_geographies <- TRUE #block
```
...
If you can't download with a code-based method, download specific geographies, and put them in the data-raw folder. For the Twin Cities, neighborhoods need to be downloaded manually.
  ...
  - [Brooklyn Park?](https://gis.brooklynpark.org/neighborhoodinfo/)
