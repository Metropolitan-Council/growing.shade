...
Growing Shade couples remote sensing data and demographic data to create insights about the intersection between the natural environment and people in real-time. This tutorial script walks through the steps necessary to create and aggregate data.

## Set parameters

Growing Shade was originally developed for the greater Twin Cities region, but should scale to other regions too. 

```{r set-params}
#What geographic region should data be created for?
state <- "MN"
counties <- c("Anoka", "Carver", "Dakota", "Hennepin", "Ramsey", "Scott", "Washington")

#Growing Shade uses census block groups for the prioritization geography. Some data sources have not yet migrated from the 2010 era census geographies to the 2020 era census geographies. When that changes, update the parameter here. 
...
forjeff <- mn_bgs %>%
  select(
                          ...
                          rename(
                            ...
                            `climchg_score` = `Climate change`,
                            `cons_score` = `Conservation`,
                            `ej_score` = `Environmental justice`,
                            `ph_score` = `Public health`
  ...
  left_join(bg_growingshade_main %>%
            ...
            by = c("GEO_ID"))
  sf::st_write(forjeff, "/Users/escheh/Desktop/growingshade_forjeff_2022.7.5.shp", append = FALSE)
...
There are different years available, different geographies, etc. 
...
# since no equity considerations yet, getting some numbers from acs github repo
acs_tractsonly <- read_csv("./20205_tract.csv") %>% #readxl::read_xlsx(unzip(temp, "CensusACSTract.xlsx")) %>%
  janitor::clean_names()
tract_acs <-  acs_tractsonly %>%
  transmute(tr20 = as.character(geog_unit),
            pd_any = anydis / cdenom,
            # povdenom = povdenom,
            tr_medianhhinc = medianhhinc, #medianhhi,
            tr_p_0017 = ageunder18 / poptotal,
            tr_p_65up = age65up / poptotal,
            tr_sens_age = tr_p_0017 + tr_p_65up,
            tr_pwk_nowork = pwk_nowork,
            tr_phhi_qntl1 = phhi_qntl1)
...
## this data uses 2010 geographies; make a crosswalk realating 2010 tracts to 2020 block groups
tract_2010 <- tigris::tracts(state = "MN", 
                                    county = c("Anoka", "Carver", "Dakota", "Hennepin", "Ramsey", "Scott", "Washington"),
                                    year = 2011)

tract_bg_crosswalk <- tract_2010 %>% transmute(tract2010_id = GEOID) %>%
  st_transform(26915) %>%
  st_buffer(-350) %>% #buffer the perimeter of the geography
  st_erase(river_lake_buffer %>%
             st_buffer(200) %>% #buffer out rivers
             st_union() %>% 
             st_buffer(0)) %>% 
  st_intersection(bg_geo  %>%
                    dplyr::select(GEOID) %>%
                    rename(bg2020_id = GEOID) %>%
                    st_transform(26915) %>% st_buffer(-20)) %>%
  mutate(overlap_area = as.numeric(st_area(.))) %>%
  st_drop_geometry()

step1 <- tract_bg_crosswalk %>% 
  group_by(bg2020_id) %>%
  mutate(n = n(),
         max_overlap = max(overlap_area))  %>%
  filter(n > 1) %>% 
  mutate(good_tract2010_id = if_else(overlap_area == max_overlap, tract2010_id, NA_character_)) %>%
  dplyr::select(tract2010_id, bg2020_id, good_tract2010_id)

step2 <- tract_bg_crosswalk %>% 
  group_by(bg2020_id) %>%
  mutate(n = n(),
         max_overlap = max(overlap_area))  %>%
  filter(n == 1) %>% 
  mutate(good_tract2010_id = tract2010_id)%>%
  dplyr::select(tract2010_id, bg2020_id, good_tract2010_id)

step4 <- tract_bg_crosswalk %>%
  full_join(step2) %>%
  full_join(step1) %>%
  full_join(census_race %>% dplyr::select(bg20, poptotal), by = c("bg2020_id" = "bg20")) %>%
  # mutate(good_tract2010_id = if_else(poptotal > 10, good_tract2010_id, NA_character_)) %>%
  filter(!is.na(good_tract2010_id)) %>%
  select(good_tract2010_id, bg2020_id)
  # count(bg2020_id) %>% filter(n>1)

## translate health data into 2020 geographies
bg_health3 <- bg_health2 %>%
  full_join(step4, by = c("tr10" = "good_tract2010_id")) %>%
  filter(!is.na(bg2020_id)) %>%
  rename(bg20 = bg2020_id) %>% dplyr::select(-tr10)
```
...
bg_growingshade_main <- bg_growingshade_data %>%
  ...
  filter(!is.na(name))#,
         # bg_string != "270539800001") #remove airport
...
filter(bg_growingshade_data, poptotal < 10)  
filter(ctu_crosswalk, tract_id == "270531044003")  

this geoid is being tricky 
filter(bg_acs,bg20 == "270539800001") %>% data.frame()
filter(bg_acs,bg20 == "270539800001")
```
