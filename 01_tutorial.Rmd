...
if (process_demographics == TRUE) {
 ...
 acs_fxn <- function(.geo){
   get_acs(
    geography = .geo, #"block group",
 ...
 acs_metadata <- get_acs(
   ...
   mutate(across(c(under18, over65), ~.x / B01001_001),  #, .names = "{.col}_percent"),
          ...
          ppov185 = (C17002_002 + C17002_003 + C17002_004 + C17002_005 + C17002_006) / C17002_001) %>%
   rename(hhincome = B19013_001) %>% 
   select(c(geo, !starts_with(c("B", "C"))))
...
- **ctu_list** which has information about the tree canopy at the city level. Columns:
...
- **nhood_list** which has information about the tree canopy at the neighborhood level. Columns:
...
bg_growingshade_data <- 
  ...
  # deal with some non-residential blocks and insufficient data; demographics specific data
      mutate(across(-c(GEOID, tract_id, land, starts_with("tr"), population_count), ~ if_else(population_count == 0, NA_real_, .x)),
...
This is the most important file of all!
...
bg_growingshade_main <- bg_growingshade_data %>%
  pivot_longer(names_to = "variable", values_to = "raw_value", -c(bg_string)) %>% #end the code after this line if you just want the reshaped data
  group_by(variable) %>%
