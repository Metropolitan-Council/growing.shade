...
<<<<<<< refs/remotes/origin/metcouncil-2021
...
>>>>>>> further generalize code
=======
state <- c("MN")
county <- NULL#c("Anoka", "Carver", "Dakota", "Hennepin", "Ramsey", "Scott", "Washington") # either type in specific county names, or for all counties within a state assign `county <- NULL`
>>>>>>> make geography work for entire states
...
if (regional_specific_data == FALSE) {
  ...
  county_outline <- if (is.null(county)) {
  tigris::counties(state = state)
  } else {tigris::counties(state = state) %>%
  filter(NAME %in% county)}
...
ctu_crosswalk <- cities %>% select(GEO_NAME) %>%
  st_transform(26915) %>%
  st_buffer(-150) %>% #buffer the perimeter of the geography
  st_intersection(bg_geo %>% 
                    dplyr::select(GEOID, ALAND, AWATER) %>%
                    st_transform(26915)) %>%
  # if less than 1% of block group falls within city boundary, don't assign it to the city
  mutate(approx_area = as.numeric(st_area(.)),
         percent = approx_area / (ALAND + AWATER)) %>%
  filter(percent > .01) %>%
  st_drop_geometry() %>%
  rename(bg_id = GEOID)
...
=======
>>>>>>> union cities geographies across counties
region_outline <- if (is.null(county)) {
  county_outline %>% group_by(STATEFP) %>%
    summarise(geometry = sf::st_union(geometry)) %>%
  sf::st_simplify(dTolerance = 400) %>%
  sf::st_transform(4326)
  } else {
    county_outline %>% group_by(COUNTYFP) %>% summarise(geometry = sf::st_union(geometry)) %>%
>>>>>>> make geography work for entire states
  sf::st_simplify(dTolerance = 400) %>%
  sf::st_transform(4326)
  }
...
# ua <- tigris::urban_areas() %>% 
#   filter(stringr::str_detect((NAME10), state)) 
# 
# # ua %>% #filter(LSAD10 == 76) %>%
# #   # filter(UATYP10 == "C") %>%
# #   ggplot()+
...
# mapview::mapview(ua)
# 
# count(st_drop_geometry(ua), LSAD10) #75 = Urbanized Area; 76 = Urban Cluster
# count(st_drop_geometry(ua), UATYP10) #Census urban area type; U = Urbanized Area; C = Urban Cluster 
# 
# # Counties can be categorized into levels of rurality based on the percentage of the decennial census population living in the rural areas. Using 2010 Census data, counties were classified as "mostly urban" (less than 50 percent of the population lived in rural areas), "mostly rural" (50 to 99.9 percent of the population lived in rural areas), and "completely rural" (100 percent of the population lived in rural areas)
# 
# # county_subdivisions
# # zctas
```

>>>>>>> identify townships in ctu geography
# Running application
...
```
=======
This is the easiest solutions that I can (currently!) think of which allows for site-specific parameters to be set *outside* of the main application code. For instance, other regions are unlikely to have a city called "Oakdale," but regardless a starting city needs to be identified in order for the map to render correctly. 
=======
Creating a `ui_params.rda` data set is the easiest solutions that I can (currently!) think of which allows for site-specific parameters to be set *outside* of the main application code. For instance, other regions are unlikely to have a city called "Oakdale," but regardless a starting city needs to be identified in order for the map to render correctly. 
>>>>>>> knit tutorial
...
# nhood_ui <- list(Minneapolis = nhood_list$GEO_NAME[nhood_list$city == "Minneapolis"],
#           `St. Paul` = nhood_list$GEO_NAME[nhood_list$city == "St. Paul"])
gs <- st_drop_geometry(nhood_list) %>% select(city, GEO_NAME) %>% group_by(city)
nhood_ui <- group_split(gs, city, keep = F) %>% purrr::flatten()#set_names(group_keys(gs))
names(nhood_ui) <- c(group_keys(gs))[[1]]
