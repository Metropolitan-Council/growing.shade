...
####
state <- c("MN")
county <- c("Anoka", "Carver", "Dakota", "Hennepin", "Ramsey", "Scott", "Washington", "Rice") # either type in specific county names, or for all counties within a state assign `county <- NULL`
...
######
regional_specific_geographies <- TRUE # go to 02_geographies.Rmd to process/make these files, as applicable
regional_specific_remotesensing <- FALSE # go to 03_remotesensing.Rmd to process/make these files, as applicable
```
...
# last updated Dec 6, 2022, but still using 2010 census geographies
...
This section doesn't seem to be easily scalable to other regions **unless zip codes are an appropriate stand-in for neighborhoods**. This "out-of-the-box" script breaks cities with >100 census blocks into "neighborhoods" using zip-codes. However, if true neighborhoods are desired, the code for this section will need to be customized (processing will be done within the `02_geographies.Rmd` script).
...
```{r city-nhood-geometries}
if (regional_specific_geographies == FALSE) {
  ...
  source_rmd(file.path(here::here(), "02_geographies.Rmd"))
...
```{r remote-sensing-random-data}
if (regional_specific_remotesensing == FALSE) {
  bg_canopy <- bg_geo %>%
    st_drop_geometry() %>%
    transmute(
      bg_id = GEOID,
  ...
  nhood_list_raw <- nhoods %>%
    ...
    source_rmd(file.path(here::here(), "03_remotesensing.Rmd"))
...
if (process_demographics == TRUE) {
  ...
  # EJ screen updated 2022-11-21; now available at the block group level and uses 2020 census geographies
#   st_drop_geometry(bg20) %>% transmute(geoid = GEOID, `2020` = T) %>%
#   full_join(st_drop_geometry(bg10) %>% transmute(geoid = GEOID10, `2010` = T)) %>%
#   filter(is.na(`2010`))
# 
# filter(ejscreen, ID == "271630705013")

    ej_url <- "https://gaftp.epa.gov/EJSCREEN/2022/EJSCREEN_2022_StatePct_with_AS_CNMI_GU_VI.csv.zip"
    download.file(ej_url, file.path(tempdir(), basename(ej_url)))
    unzip(zipfile = file.path(tempdir(), basename(ej_url)), exdir = tempdir())
    
    ejscreen <- readr::read_csv(file.path(tempdir(), stringr::str_remove(basename(ej_url), ".zip")),
                         col_select = c(ST_ABBREV, CANCER, ID)) %>%
      filter(ST_ABBREV == state)
