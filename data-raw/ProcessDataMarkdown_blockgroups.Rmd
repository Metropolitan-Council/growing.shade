...
#confirm that some areas have no tree cover
# test <- mn_bgs_2 %>% 
#   filter(GEOID == "270759901000")
# leaflet() %>%
#   addTiles() %>%
...
# test to see that we have tree canopy percent for all of MN and WI collar counties; this is a pretty big map to load fyi
# test <- mn_bgs_1 %>% full_join(bg_canopy, by = c("GEOID" = "GEOID10"))
# pal <- colorNumeric(
#   palette = "Greens",
#   domain = test$canopy_percent)
# leaflet() %>%
#   addTiles() %>%
#   addPolygons(data = test %>% st_transform(4326),
#               color = ~pal(canopy_percent),
...
#   filter(bg_growingshade_data, bg10 == "270030501071") %>%
#     full_join(
