...
usfs <- readxl::read_xlsx("../data-raw/evalidator fsusda.xlsx", 
  ...
  mutate(age2 = factor(age, levels=c("0-20 years",
                                    "21-40 years",
                                    "41-60 years",
                                    "61-80 years",
                                    "81-100 years",
                                    "100+ years")),
         label = case_when(age2 == "100+ years" ~ sps,
                           TRUE ~ NA_character_),
...
geom_line(lwd =1) +
  ...
  ggrepel::geom_label_repel(
    aes("100+ years", carbon, label = label), col = "black", nudge_x = .5, direction = "y", hjust = "left",  ylim = c(-Inf, Inf),  xlim = c(NA, Inf)
  ) +
  scale_x_discrete(expand = expansion(mult = c(.05, 0.6))) +
  scale_shape_manual(values = rep((21:25), 3)) +
  guides(fill = F, col = F, shape = F) +
  labs(x = "Stand age", y = "Average\ncarbon\nstorage\n(US tons\nper acre)",
...
ggsave("./usfs_fig", usfs_fig,  width = 8, height = 5, units = "in", device = "png")
